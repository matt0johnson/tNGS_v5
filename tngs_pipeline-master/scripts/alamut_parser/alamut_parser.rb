  ...
  def parse_unwanted_variants(unwanted_file_path)
	...
	def populate_metrics(samples, this_batch, this_parser)
		...
		samples.each do |this_sample|
			...
			phenotype_metric_file_path = "#{this_batch['base_path']}/#{this_batch['batch_id']}/metrics/hsmetrics/#{this_sample.panel_version}_#{this_sample.ex_number}-#{this_sample.gender.upcase!}-#{this_sample.phenotype}_phenotype_bait_capture_metrics"
	...
	def run_parser(parser)
		...
		base_path = path.split("/scripts/").first
  		...
  		this_batch = YAML.load_file("../../snakefiles/config.yaml")
		  ...
		  #parser.base_path = this_batch['workbatch']
  		parser.sample_list_path = this_batch['sample_list_path']
  		
  		
  		samples = parser.parse_sample_list("../../snakefiles/#{parser.sample_list_path}")
...
####		
  		this_book.add_worksheet(:name => "Batch") do |this_sheet|
  				...
  				this_sheet.add_row ["FTP url", "#{this_batch['ftp']}"]
  			...
  			#	this_sheet.add_row ["Validation date", "#{this_batch.validation_date}"]
  				...
  				this_sheet.add_row ["Alamut path", "#{this_batch['alamut']}"]
  				this_sheet.add_row ["Java path", "#{this_batch['java']}"]
  				this_sheet.add_row ["Rscript path", "#{this_batch['rscript']}"]
  				this_sheet.add_row ["BWA path", "#{this_batch['bwa']}"]
  				this_sheet.add_row ["Picard path", "#{this_batch['picard']}"]
  				this_sheet.add_row ["GATK path", "#{this_batch['gatk']}"]
  				this_sheet.add_row ["Reference path", "#{this_batch['reference_genome']}"]
  				this_sheet.add_row ["Common variant NKMI path", "#{this_batch['common']}"]
  				this_sheet.add_row ["Common artefacts path", "#{this_batch['artefacts']}"]
...
#####  	
		...
		axlsx_package.serialize "#{parser.base_path}/#{parser.batch_id}/variants/#{parser.batch_id}_variants.#{Time.now.strftime("%d-%m-%Y-%H%M%S")}.xlsx"
		
		File.open("#{parser.base_path}/#{parser.batch_id}/variants/#{parser.batch_id}_variants.yaml", 'w+') {|f| f.write(sample_store.to_yaml)}
		#File.open("#{parser.base_path}/variants/variants.yaml", 'w+') {|f| f.write(sample_store.to_yaml)}
