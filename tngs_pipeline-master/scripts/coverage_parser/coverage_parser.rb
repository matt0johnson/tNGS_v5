...
class CoverageParser
  ...
  def parse_metrics(metrics_file_path)
	...
	def write_sample_worksheet(this_sample, this_book, this_batch)
  		...
  		this_panel = this_batch["panels"][sample_panel_version]
  		...
  		coverage_array = local_parser.load_coverage("#{base_path}/#{batch_id}/metrics/phenotypecoverage/#{panel_id}_#{this_sample.ex_number}-#{this_sample.phenotype}_phenotypecoverage", temp_sample_id, coverage_cutoff)
  		puts "Loaded coverage : #{coverage_array.length} records"
  		
  		interval_array = local_parser.load_bed_intervals("#{base_path}/#{batch_id}/#{intervals_directory}/#{panel_id}_#{this_sample.phenotype}_metrics.bed")
  		...
  		if (coverage_array != nil) && (interval_array != nil)
  		  ...
  		  file_name = "#{base_path}/#{batch_id}/metrics/phenotypecoverage/#{panel_id}_#{this_sample.ex_number}_#{this_sample.phenotype}_metrics.less_than_30_coverage"
  ...
  if __FILE__ == $PROGRAM_NAME
  	...
  	#Load the config YAML file and pass the settings to local variables
 		...
 		this_batch = YAML.load_file('../../snakefiles/config.yaml')
  		...
  		metrics_file_path = "../../metrics/#{this_batch["batch_id"]}_overall_metrics"
  		...
  		samples = parser.parse_sample_list("#{this_batch["base_path"]}/#{this_batch["batch_id"]}/snakefiles/#{this_batch["sample_list_path"]}")
  		...
  		genesets.each do |this_geneset|
  			phenotype_metric_file_path = "#{this_batch["base_path"]}/#{this_batch["batch_id"]}/metrics/#{this_batch["batch_id"]}_#{this_geneset}_phenotype_metrics"
  		...
  		this_book.write "#{this_batch["base_path"]}/#{this_batch["batch_id"]}/variants/#{this_batch["batch_id"]}_metrics_coverage.#{Time.now.strftime("%d-%m-%Y-%H%M%S")}.xls"
