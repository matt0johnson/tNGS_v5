...
class SampleStore
    ...
    def capture_numbers
  	...
  	def select_panel(batch, sample)
          ...
          if this_panel[1]['panel_id'] == sample.panel_version
  	...
  	def process_samples(parser, this_batch, this_book, title_font)
        ...
        transcripts_path = "#{this_batch['base_path']}/#{this_batch['batch_id']}/" + this_panel[1]['transcript_file_path']
        transcript_store = parse_transcripts(parser, transcripts_path)
  			
  			if !all_transcripts.keys.include?(this_panel[1]['panel_version'])
  				all_transcripts[this_panel[1]['panel_version']] = transcript_store
  			end
  			...
  			unwanted_variants = parser.parse_unwanted_variants("#{this_batch['base_path']}/#{this_batch['batch_id']}/#{this_panel[1]['unwanted_file_path']}")
  			#Load wanted regions
  			wanted_regions = parser.parse_wanted_regions("#{this_batch['base_path']}/#{this_batch['batch_id']}/#{this_panel[1]['wanted_file_path']}")
  			...
  			#Parse CNV files
  			if ( ["MALE","FEMALE"].include?(gender.upcase) ) && ( ["v5","v501","v602","v603"].include?(this_panel[1]['panel_id']) )
  				cnv_array = parser.parse_cnvs("#{this_panel[1]['panel_id']}", "#{this_batch['base_path']}/#{parser.batch_id}/variants/#{this_panel[1]['panel_id'].downcase}_#{this_sample.ex_number}-#{gender}_exomedepth.csv")
          self.cnv_store.cnvs.store("#{this_sample.ex_number}", cnv_array)
  				...
  				puts "#{this_sample.capture_number} CNV file will not be parsed: check that gender(#{gender}) and panel (#{this_panel[1]['panel_id']}) meet criteria for running ExomeDepth"
  			end
  			

  			#Load variants from alltrans alamut file	
  			variants = parser.parse_file("#{this_batch['base_path']}/#{parser.batch_id}/variants/#{this_panel[1]['panel_id']}_#{this_sample.ex_number}-#{gender}-#{phenotype}_annotated.txt", "#{this_panel[1]['panel_id'].downcase}", "#{this_sample.ex_number.to_s.downcase}")
